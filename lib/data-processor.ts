...
export function prepareGroupedBarData(
  ...
  return years.map(year => {
    ...
    records.forEach(record => {
        ...
        // Check if we need Global-to-regional mapping for stacked bars
        const regionalGeographiesStacked = ['North America', 'Europe', 'Asia Pacific', 'Latin America', 'Middle East', 'Africa', 'Middle East & Africa', 'ASEAN', 'SAARC Region', 'CIS Region']
        const hasRegionalSelectionStacked = geographies.some(g => regionalGeographiesStacked.includes(g))
        const hasOnlyGlobalRecordsStacked = records.every(r => r.geography === 'Global')
        const needsGlobalMappingStacked = hasRegionalSelectionStacked && hasOnlyGlobalRecordsStacked && !geographies.includes('Global')

        // Realistic regional market share distribution for stacked bars
        const regionalMarketSharesStacked: Record<string, number> = {
          'North America': 0.32,
          'Europe': 0.28,
          'Asia Pacific': 0.25,
          'Latin America': 0.08,
        ...
        records.forEach(record => {
          ...
          // Handle Global data distribution to selected regional geographies
          if (needsGlobalMappingStacked && geography === 'Global') {
            const selectedRegionals = geographies.filter(g => regionalGeographiesStacked.includes(g))
            const selectedShareSum = selectedRegionals.reduce((sum, region) =>
              sum + (regionalMarketSharesStacked[region] || 0.1), 0
            )
            const recordValue = record.time_series[year] || 0

            // Distribute to each selected regional geography
            selectedRegionals.forEach(region => {
              const regionShare = regionalMarketSharesStacked[region] || 0.1
              const normalizedShare = regionShare / selectedShareSum
              const distributedValue = recordValue * normalizedShare

              if (!geoMap.has(region)) {
                geoMap.set(region, new Map())
              }
              const segmentMap = geoMap.get(region)!
              const currentValue = segmentMap.get(segment) || 0
              segmentMap.set(segment, currentValue + distributedValue)
            })
            return // Skip normal processing since we handled Global distribution
          }
...
export function prepareIntelligentMultiLevelData(
  ...
  return years.map(year => {
    ...
    if (needsGlobalMapping) {
      ...
      // When multiple segments are selected, we need to sum values from ALL segments
      // Group records by segment first to avoid double-counting, then sum the best record per segment
      let globalTotal = 0
      segmentGroups.forEach((groupRecords) => {
        // Group records by segment to handle multiple segments correctly
        const recordsBySegment = new Map<string, DataRecord[]>()
        groupRecords.forEach(record => {
          const segmentKey = record.segment
          if (!recordsBySegment.has(segmentKey)) {
            recordsBySegment.set(segmentKey, [])
          }
          recordsBySegment.get(segmentKey)!.push(record)
        })

        // For each segment, pick the best record (leaf preferred) and add to total
        recordsBySegment.forEach((segmentRecords) => {
          const leafRecord = segmentRecords.find(r => !r.is_aggregated)
          if (leafRecord) {
            globalTotal += leafRecord.time_series[year] || 0
          } else {
            const bestRecord = segmentRecords.reduce((best, current) => {
